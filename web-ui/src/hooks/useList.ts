...
export interface ListActions<T> {
   ...
   */
  updateAt: (index: number, item: T | ((item: T) => void)) => void;
  updateBy: (predicate: (a: T) => boolean, item: T | ((item: T) => void)) => void;
  /**
...
function useList<T>(initialList: IHookStateInitAction<T[]> = []): [T[], ListActions<T>] {
  ...
  const actions = useMemo<ListActions<T>>(() => {
    const a = {
      ...
      updateAt: (index: number, item: T | ((item: T) => void)) => {
        actions.set((curr: T[]) => {
      ...
      updateBy: (predicate: (a: T) => boolean, item: T | ((item: T) => void)) => {
        actions.set((curr: T[]) => {
          const index = curr.findIndex(predicate)
          if (index == -1) {
            return curr
          }
          const arr = curr.slice();
